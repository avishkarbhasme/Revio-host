...
const getAllVideos = asyncHandler(async (req, res) => {
  const { page = 1, limit = 10, query, sortBy, sortType, userId, filter } = req.query;

  let match = { isPublished: true }; // Default: show all published
  ...
  // --- Add time filter ---
  if (filter && filter !== "latest" && filter !== "oldest") {
    ...
    switch (filter) {
      case "6hr":
        ...
        break;
      case "12hr":
        dateThreshold.setHours(dateThreshold.getHours() - 12);
        break;
      case "1day":
  ...
  const aggregate = Video.aggregate([
    ...
    // Join owner info from 'users' collection
    {
      $lookup: {
        from: "users",
        localField: "owner",
  ...
  // --- Add sorting ---
  let sortOption = {};
  if (filter === "latest" || !filter) sortOption.createdAt = -1;
  else if (filter === "oldest") sortOption.createdAt = 1;
  else sortOption[sortBy || "createdAt"] = sortType === "asc" ? 1 : -1;

  aggregate.sort(sortOption);
...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    if(!videoFileLocalPath){
        throw new ApiError(400,"videoFileLocalPath is requried")
    }
    ...
    if(!thumbnailLocalPath){
        throw new ApiError(400,"thumbnailLocalPath is requried")
    }
    ...
    if(!videoUploaded){
        throw new ApiError(500, "Something went wrong while videoUpload ");
    }

    return res
    .status(200)
    .json(
        new ApiResponse(200,videoUploaded,"videoFIle is uploaded succesfully")
    )
...
// ✅ check if user has already viewed it
  ...
  if (req.user?._id) {
    alreadyViewed = await User.exists({
      _id: req.user._id,
